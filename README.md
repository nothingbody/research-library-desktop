...
当前版本：**0.9.30**。项目仍在开发中，升级前请先备份个人文献库。软件不是 Scholay、知网、Elsevier、Zotero 或其他学术平台的官方客户端。
...
| 模块 | 当前能力 |
| --- | --- |
| 文献库 | 导入题录与 PDF、重复检测、集合与标签、智能集合、高级检索、备份与恢复 |
| 文献发现 | 从 OpenAlex、Crossref 等公开书目来源检索；可选 PubMed、arXiv；展示来源、摘要可用性、年份和被引数据，支持筛选、排序和批量保存 |
| 浏览器采集 | Chrome / Edge 扩展采集论文页面题录；核对后保存到本机，有可获取的公开 PDF 时可下载和索引 |
...
安装版中可从“设置 → 浏览器采集”打开扩展目录。在 Chrome 的 `chrome://extensions` 或 Edge 的 `edge://extensions` 开启开发者模式，点击“加载已解压的扩展程序”并选择该目录。保持桌面端运行，扩展会自动连接本机文献库，**无需配对码**。网页缺少完整题录时，请在弹窗中补全后保存。
...
- **无法获取 PDF**：论文页面可能只是落地页，或需要付费/机构权限。可以先保存题录，再手动添加自己有权使用的 PDF。
