...
当前版本：**0.9.32**。项目仍在开发中，升级前请先备份个人文献库。软件不是 Scholay、知网、Elsevier、Zotero 或其他学术平台的官方客户端。
...
安装版中可从“设置 → 浏览器采集”打开扩展目录。在 Chrome 的 `chrome://extensions` 或 Edge 的 `edge://extensions` 开启开发者模式，点击“加载已解压的扩展程序”并选择该目录。保持桌面端运行，扩展会自动连接本机文献库，**无需配对码**。网页缺少完整题录时，请在弹窗中补全后保存。扩展会优先读取论文页的 PDF 元数据，并识别 Springer Nature、Wiley、Taylor & Francis、SAGE、ACS、ACM、MDPI、Frontiers、PLOS、PMC、Oxford、Cambridge、IEEE 和 ScienceDirect 的文章 PDF 入口；其中部分站点使用当前浏览器会话下载，验证文件确实是 PDF 后再导入。浏览器下载的原文件仍会留在系统下载目录。
