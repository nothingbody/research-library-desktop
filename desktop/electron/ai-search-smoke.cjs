...
exports.run = async ({win, rpc, root, project, app}) => {
  ...
  try {
    ...
    assert.equal(await run('getComputedStyle(document.querySelector(".ai-session-rail")).display'), 'none');
    assert.equal(await run('document.querySelector(".discovery-result-filters select").value'), 'screening');
    await capture('02-simple-results');
    await run('document.querySelector(".discovery-list thead input").click()');
    await until('!!document.querySelector(".discovery-selection-bar")', 'visible batch actions');
    await capture('02b-selected-results');
    await run('document.querySelector(".discovery-selection-bar button:last-child").click()');
    ...
    assert.ok(await run('document.querySelector(".discovery-primary-actions").getBoundingClientRect().top < document.querySelector(".discovery-intro").getBoundingClientRect().top'));
